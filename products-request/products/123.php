...
$query = "SELECT * FROM products_request WHERE product = 'Adonko 123' ORDER BY ID DESC";
...
$number_of_finished_products = get_qty($ims_products_connect, "Adonko 123");
$total_number_of_products_requested = mysqli_fetch_assoc(mysqli_query($procurement_connect, "SELECT SUM(qty) FROM products_request WHERE product = 'Adonko 123' AND is_approved = '0' ORDER BY ID DESC"))['SUM(qty)'];
if ($number_of_finished_products > $total_number_of_products_requested) {
    $product_deficiency = 0;
} else {
    $product_deficiency = abs(get_qty($ims_products_connect, "Adonko 123") - $total_number_of_products_requested);
}
                        ...
                        <div class="white-box analytics-info" style="display: flex; height: 85%;">
                            <ul class="list-inline two-part d-flex align-items-center justify-content-around mb-0">
                                ...
                                <li style="margin-left: 200px;">
                                    <span class="counter text-success">Adonko 123</span>
                                </li>
                            </ul>
                        </div>
                    </div>
