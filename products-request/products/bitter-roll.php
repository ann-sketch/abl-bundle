...
function calc_raw_stock($ims_connect)
{
    ...
    $box_qty = mysqli_fetch_assoc(mysqli_query($ims_connect, "SELECT * FROM `products` WHERE `name` = 'CARTON BITTERS 50ML'"))['qty'];
...
$total_number_of_products_requested = mysqli_fetch_assoc(mysqli_query($procurement_connect, "SELECT SUM(qty) FROM products_request WHERE product = 'Adonko Bitters (Roll)' AND is_approved = '0' ORDER BY ID DESC"))['SUM(qty)'] ?? 0;
