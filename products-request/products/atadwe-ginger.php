...
$query = "SELECT * FROM products_request WHERE product = 'Adonko Atadwe Ginger (Bottle)' ORDER BY ID DESC";
...
function calc_raw_stock($ims_connect)
{
    ...
    $box_qty = mysqli_fetch_assoc(mysqli_query($ims_connect, "SELECT * FROM `products` WHERE `name` = 'CARTON ATADWE GINGER 750 ML'"))['qty'];
...
$number_of_finished_products = get_qty($ims_products_connect, "Adonko Atadwe Ginger (Bottle)");
$total_number_of_products_requested = mysqli_fetch_assoc(mysqli_query($procurement_connect, "SELECT SUM(qty) FROM products_request WHERE product = 'Adonko Atadwe Ginger (Bottle)' AND is_approved = '0' ORDER BY ID DESC"))['SUM(qty)'] ?? 0;
if ($number_of_finished_products > $total_number_of_products_requested) {
    $product_deficiency = 0;
} else {
    $product_deficiency = abs(get_qty($ims_products_connect, "Adonko Atadwe Ginger (Bottle)") - $total_number_of_products_requested);
}
                        ...
                        <div class="white-box analytics-info" style="display: flex; height: 85%;">
                            <ul class="list-inline two-part d-flex align-items-center justify-content-around mb-0 flex-wrap">
                                ...
                                <li style="margin-left: 50px;">
                                    <span class="counter text-success">Adonko Atadwe Ginger (Bottle)</span>
                                </li>
                            </ul>
                        </div>
                    </div>
