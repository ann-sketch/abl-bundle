...
$query = "SELECT * FROM products_request WHERE product = 'Adonko Dry Gin (Roll)' ORDER BY ID DESC";
...
$number_of_finished_products = get_qty($ims_products_connect, "Adonko Dry Gin (Roll)");
$total_number_of_products_requested = mysqli_fetch_assoc(mysqli_query($procurement_connect, "SELECT SUM(qty) FROM products_request WHERE product = 'Adonko Dry Gin (Roll)' AND is_approved = '0' ORDER BY ID DESC"))['SUM(qty)'] ?? 0;
if ($number_of_finished_products > $total_number_of_products_requested) {
    $product_deficiency = 0;
} else {
    $product_deficiency = abs(get_qty($ims_products_connect, "Adonko Dry Gin (Roll)") - $total_number_of_products_requested);
}
                        ...
                        <div class="white-box analytics-info" style="display: flex; height: 85%;">
                            <ul class="list-inline two-part d-flex align-items-center justify-content-around mb-0 flex-wrap">
                                ...
                                <li style="margin-left: 50px;">
                                    <span class="counter text-success">Adonko Dry Gin (Roll)</span>
                                </li>
                            </ul>
                        </div>
                    </div>
